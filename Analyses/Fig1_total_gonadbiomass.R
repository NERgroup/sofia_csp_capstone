...
#convert size (diameter) to biomass 
...
#calculate individual gonad mass from biomass and GSI
...
gonad_mass_site_total <- gonad_mass_site_zone %>%
  ...
  mutate(site_type = word(site_id_final, 2),
         t_gonad_mass_kg = t_gonad_mass/1000, #per 80m2
         t_gonad_mass_sim_kg = t_gonad_mass_sim/1000, #per80m2
         t_gonad_mass_sim_g_m2 = t_gonad_mass_sim/80,
         t_biomass_sim_g_m2 = t_biomass_g/80) %>% 
  filter(site_type %in% c("BAR", "FOR","INCIP"))

#total urchins x total gonad mass (natural variability) per 80m2
plot(t_gonad_mass_sim_kg~n_urch, data = gonad_mass_site_total)

#total urchins x total gonad mass (natural variability) per 80m2 with regressions
ggplot(gonad_mass_site_total %>% filter(site_type != "INCIP"), aes(x = n_urch, y = t_gonad_mass_sim_kg, fill = site_type))+
  geom_point()+
  geom_smooth(method = "loess", span = 1)

#total biomass x total gonad mass 80m2
ggplot(
  gonad_mass_site_total %>% filter((site_type != "INCIP"),t_biomass_kg < 80),
  ...
  scale_color_manual(
    ...
    labels = c("BAR" = "Barren", "FOR" = "Forest"))) +
  labs(
    x = "Total Biomass (kg/80m2)",
    y = "Total Gonad Mass (kg/80m2)") +
  theme_classic() +
  theme(
...
#total biomass x total gonad mass (without incipient) per m2
ggplot(
  gonad_mass_site_total %>% filter((site_type != "INCIP"), t_gonad_mass_sim_g_m2 <80),
